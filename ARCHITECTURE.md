...
### 6.1 Residual-risk migration strategy

The recommended migration order for unsettled residual risks is:

1. **Security first.** Implement and enforce the remaining authorisation and audit-boundary gates before any further feature work. The immediate priorities are `no-self-authorisation-check`, `audit-append-port-only-check`, and `sop-template-admin-port-only-check`, with `no-self-authorisation-check` treated as the aggregate control that closes the self-elevation path.
2. **Traceability second.** Implement `audit-traceability-check` so the generated traceability index and the lightweight file headers remain synchronised. This makes the release auditable rather than merely testable.
3. **Promote already-green controls.** Move gate rows and board entries from `informational` or `not_implemented` to `enforced` only after the owning task is implemented and the corresponding CI evidence is green. Catalogue staleness, source-grade citations, rule-fixture coverage, task-acceptance completeness, module coverage, and gate lifecycle should be promoted in this order when their predicates are real.
4. **Handle format migration explicitly.** Treat SBOL and coordinate round-trip changes as semantic migrations, not byte-for-byte rewrites. Preserve canonical hashes, ship golden round-trip fixtures, and fail fast on lossy conversion unless a documented compatibility shim exists.
5. **Govern maintenance.** Add recurring review windows for catalogues and related metadata. Monthly reviews are appropriate for SR/screening-sensitive material; quarterly reviews are appropriate for MR/WR/BR/MS. Record the review result in the governance/audit trail.
