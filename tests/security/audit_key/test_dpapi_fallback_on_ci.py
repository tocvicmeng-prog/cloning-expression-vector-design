"""
module_id: tests.security.audit_key.test_dpapi_fallback_on_ci
file: tests/security/audit_key/test_dpapi_fallback_on_ci.py
task_id: T-312b
"""
...
from adapter.security.audit_key import WindowsDpapiAuditKeyProvider
from tests.security.audit_key.helpers import admin_principal, deterministic_key_factory


def test_windows_dpapi_backend_falls_back_to_file_keystore_on_ci(
    tmp_path: Path,
    ...
    with pytest.warns(RuntimeWarning, match="CI environment"):
        provider = WindowsDpapiAuditKeyProvider(
