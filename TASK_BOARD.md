...
**Last updated:** 2026-05-14 (T-704 primer designer complete locally; project is a Git repository on `main` and uses non-editable `uv` installs for Windows/OneDrive/non-ASCII path reliability).
...
**Next milestone.** Phase 7 — `T-705` assembly orchestrator is next. Phase 2 scaffold (`T-201`..`T-205`), all Phase 3 tasks (`T-301`..`T-315`), all Phase 4 tasks (`T-401`..`T-406`, `T-316c`, `T-316b`), all Phase 5 tasks (`T-501`, `T-502`, `T-504`, `T-503`), all Phase 6 tasks (`T-601a..k`, `T-602`, `T-603`, `T-606`, `T-607`), and T-701 through T-704 are done + locally verified.
...
| Phase 7 — Codon + assembly + overhang + primer | 🟡 | 4 / 5 | T-701 codon optimiser, T-702 overhang optimiser, T-703 assembly strategy hierarchy, and T-704 primer designer verified locally. Next: T-705 assembly orchestrator. |
...
**Cumulative (v1.5, seed-manifest checked):** 9 foundation items done + 45 implementation tasks done / **71 active Section 2 implementation task cards**. The fifth-round audit's stale legacy profile-signing heading is removed; `T-601a..k` remains one active card with formal range expansion for child briefs. Phase 0–6 complete locally; Phase 7 in progress; next task T-705.
...
_None at present — Phase 7 is unblocked. The orchestrator can open `T-705` immediately._
...
_Empty — no tasks assigned yet. Phase 7 next task is `T-705`._
