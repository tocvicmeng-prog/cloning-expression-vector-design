...
| **Phase 4.2 — Catalogue extension (v0.2)** | 🟡 | **6 / 10** | T-407 ✅ host schema v1.0 → v1.1; T-408 ✅ markers schema v1.0; T-409 ✅ MarkersCataloguePort; T-410 ✅ **23 markers populated with primary-lit citations**; T-411 ✅ **9 new strain records, host-marker links resolve**; T-415 ✅ CI gates (markers-citation + host-marker-link now actively enforcing real data); **remaining**: T-412 + T-413 (engine migrations), T-414 (MR-55..60 predicates), T-408a (existing-record backfill). |
...
| **Phase 14 — ML training corpus subsystem (v0.2)** | 🟡 | **9 / 11** | T-1401 ✅ scaffold + import-linter contract; T-1402 ✅ corpus_record schema; T-1403 ✅ license CI gate; T-1406 ✅ BR-16 snapgene-pipeline-scan enforced; T-1407 ✅ partition router + manifest generator; T-1408 ✅ annotation-provenance heuristic informational; T-1409 ✅ release gate (research + commercial modes); T-1410 ✅ IP_POLICY.md + LICENSES/THIRD_PARTY_NOTICES.md; T-1411 ✅ fork-readiness operational checklist; **remaining**: T-1404 (Opus, 40 backbone records), T-1405 (Opus, 80 modular element records). |

**Cumulative (v1.5 + v0.2 Enrichment Amendment, seed-manifest checked):** 9 foundation items done + 71 v0.1.0 implementation tasks done + 2 v0.2 tasks in-progress (T-407 + T-1401, M-A entry points) / **92 active Section 2 implementation task cards across 16 phases**. The fifth-round audit's stale legacy profile-signing heading is removed; `T-601a..k` remains one active card with formal range expansion for child briefs. **Phases 0–13 complete locally; Phases 4.2 + 14 in M-A startup as of 2026-05-23.**
...
| **M-B** — Core implementations | T-409, T-410 (Opus), T-411 (Opus), T-412, T-413, T-414, T-1403, T-1404 (Opus), T-1405 (Opus) | (begins on M-A exit) | 23 markers committed; 9 new strain records committed; ~120 corpus records committed; engine migrations green; dual-read shim hit-rate baseline established; MR-55..60 fire correctly on fixtures. **Compression checkpoint between M-B and M-C** — heaviest curation work; context reset recommended. |
| **M-C** — CI gates + policy (release-readiness) | T-415, T-408a, T-1407, T-1408 (informational at v0.2), T-1409, T-1410, T-1411 (Opus) | (begins on M-B exit) | All six new CI gates green and lifecycle-stamped; `corpus_manifest.yaml::license_aggregate` populated; `partition: sa_free` default; fork-readiness operational checklist drafted; `IP_POLICY.md` + `LICENSES/THIRD_PARTY_NOTICES.md` landed. |
...
**Status summary:** **15 tasks ✅ done** (T-407, T-1401, T-408, T-1402, T-1406 from M-A; T-409, T-1410, T-1411, T-1403, T-415, T-1407, T-1408, T-1409, T-410, T-411 from M-B/M-C — all completed 2026-05-23), **6 tasks ⚪ planned**. v0.1.0 baseline (71 tasks) all ✅ done — see § 4. **🎯 M-A COMPLETE. M-B substantially advanced: 2 of 4 Opus curation blocks (T-410 markers, T-411 strains) landed at A-grade citation quality; cross-catalogue integrity (host→marker links) actively enforcing.**

**Final honest stop point 2026-05-23.** Remaining 8 tasks all require either dedicated scientific-curation discipline or careful engine-migration review:
- **T-410** — Opus, 23 markers content with Sambrook + primary-lit citations
- **T-411** — Opus, 9 new strain records with full v1.1 phenotype field population
- **T-408a** — Sonnet, backfill 6-8 existing host records (depends on T-410+T-411)
- **T-412** — Sonnet, `engine.compatibility` migration to MarkersCataloguePort with dual-read shim (RISKY against 71-card v0.1.0 baseline)
- **T-413** — Sonnet, `engine.validation` migration with dual-read shim (RISKY, same)
- **T-414** — Sonnet, 6 MR rule predicates (MR-55..60) + 12 fixtures (gated on T-410+T-411)
- **T-1404** — Opus, 40 backbone corpus records with NCBI/EBI/iGEM/vendor primary-lit provenance
- **T-1405** — Opus, 80 modular element corpus records

Each of these is best executed in a fresh focused session with the relevant primary-literature access. The CI gates landed at this push (T-1403, T-1408, T-415's markers + host-link gates) will switch from `informational` vacuous-pass mode to `enforced` active-checking mode AUTOMATICALLY as the curation tasks land their content. The release gate (T-1409) and partition router (T-1407) are ready for the first corpus-ingestion batch.
...
| **T-408a** | Existing host-record v1.1 backfill | M-C | Sonnet | T-407, T-410, T-411 | ⚪ planned |
...
| **T-412** | `engine.compatibility` migration to `MarkersCataloguePort` (dual-read shim) | M-B | Sonnet | T-409, T-410, T-411 | ⚪ planned |
| **T-413** | `engine.validation` migration to `MarkersCataloguePort` | M-B | Sonnet | T-409, T-412 | ⚪ planned |
| **T-414** | MR-55..60 rule predicates (methylation, disulfide, protease, rare-codon, marker-host aux, Y2H) | M-B | Sonnet | T-410, T-411, T-413 | ⚪ planned |
...
| **T-1404** | Backbone corpus seed (~40 records: pUC, pBR322, pET, pBAD, pCDF, pRSF, pACYC, pGEX, pMAL, pTrc, pPICZ, pPIC, pAO815, pYES2, pRS, pcDNA, pLenti, pAAV, pCMV, pIRES, pHR') | M-B | **Opus** | T-1402, T-1403 | ⚪ planned |
| **T-1405** | Modular-element corpus seed (~80 records: promoters, terminators, RBS/Kozak, polyA, IRES/2A, MCS, tags, FPs, selection cassettes, insulators, introns/WPRE) | M-B | **Opus** | T-1402, T-1403 | ⚪ planned |
