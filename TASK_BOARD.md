...
**Last updated:** 2026-05-14 (T-802 design-plan generator complete locally; Phase 8a in progress; project is a Git repository on `main` and uses non-editable `uv` installs for Windows/OneDrive/non-ASCII path reliability).
...
**Next milestone.** Phase 8a — `T-804` controls generator is next. Phase 2 scaffold (`T-201`..`T-205`), all Phase 3 tasks (`T-301`..`T-315`), all Phase 4 tasks (`T-401`..`T-406`, `T-316c`, `T-316b`), all Phase 5 tasks (`T-501`, `T-502`, `T-504`, `T-503`), all Phase 6 tasks (`T-601a..k`, `T-602`, `T-603`, `T-606`, `T-607`), all Phase 7 tasks (`T-701`..`T-705`), and T-801/T-802 are done + locally verified.
...
| **Phase 8a** — Design plan + controls + advisory data + advisory presentation (pre-screening) | 🟡 | 2 / 7 | T-801 and T-802 verified locally; next T-804, then **T-805a (`app.design_plan_orchestrator` — v1.2 new per B2-09)**, T-806a, T-807, T-808 |
...
**Cumulative (v1.5, seed-manifest checked):** 9 foundation items done + 48 implementation tasks done / **71 active Section 2 implementation task cards**. The fifth-round audit's stale legacy profile-signing heading is removed; `T-601a..k` remains one active card with formal range expansion for child briefs. Phase 0–7 complete locally; Phase 8a is in progress; next task T-804.
...
_None at present — Phase 8a is unblocked. The orchestrator can open `T-804` immediately._
...
_Empty — no tasks assigned yet. Phase 8a next task is `T-804`._
...
| R-12 | Junior researcher misuses generated protocol | 🟡 non-operational design-plan rendering delivered in T-802; gated SOP rendering and SOP-after-gates CI gate planned in T-803 / T-204 |
...
| R-21 | Advisory bypass | 🟡 report generation delivered in T-801; `no-passive-advisory-bypass-check` CI gate planned in T-806; FR-ADV-07 adversarial UAT planned in T-1302 |
