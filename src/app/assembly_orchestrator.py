"""
module_id: app.assembly_orchestrator
file: src/app/assembly_orchestrator.py
task_id: T-203

Public API placeholder for app.assembly_orchestrator. The owning implementation task is T-705.
"""

from __future__ import annotations

from typing import NoReturn

MODULE_ID = "app.assembly_orchestrator"
OWNING_TASKS = ("T-705",)


class PublicApiStub:
    """Marker class replaced by the owning implementation task."""


def module_not_implemented() -> NoReturn:
    """Raise until app.assembly_orchestrator's public API is implemented by its owning task."""
    raise NotImplementedError(f"{MODULE_ID} public API is scheduled in {OWNING_TASKS}")
