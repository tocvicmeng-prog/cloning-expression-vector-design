...
- **`agenda_consistency_check.py` updates.** Heading parser regex extended to accept `4.2` as a phase token; `EXPECTED_TOTAL` 71 → 91; `EXPECTED_PHASES` adds `"4.2"` (after `"4"`) and `"14"` (at end); `EXPECTED_COUNTS` adds `"4.2": 10` + `"14": 11`; `EXPECTED_PHASE_4_2` + `EXPECTED_PHASE_14` task lists added; canonical-port-text checks bumped 50 → 51; `STALE_IDS` adds `"T-MARKERS-SHIM"`.
- **task_manifest.yaml updates.** `active_task_card_count` 71 → 91; `phase_order` extended with `"4.2"` and `"14"`; `phase_counts` extended; 20 new task rows appended in heading order.
...
- **Acceptance:** both new gates `enforced` and green; port-count consistency check fires on a deliberately desynchronised fixture and passes on the real artefacts; agenda check green at `91 active task headings, 51 canonical ports`. (Note: actual count is **92** not 91 — joint-plan off-by-one reconciled at step 9 cadence close.)
