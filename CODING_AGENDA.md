...
- **Files:** `src/app/export_orchestrator.py`, `redaction.py`, `src/app/export_orchestrator/renderers/{bundle_zip,manifest}.py` (v1.2), tests.
- **Model tier:** Opus. **Context budget:** ≤ 50 k tokens.
- **SOP (v1.2):**
...
- **Acceptance:** all 17 scenarios green; each produces the expected typed denial; each writes the expected audit row through the broker (engine-process) or admin handler (admin-service); none silently succeeds.
...
| `events/export/<institution>.jsonl` | `app.export_orchestrator` (T-903) | release-management, retrospective audits | `BundleEmitted`, `VendorSubmissionPrepared`, `ExportProfileRedactionApplied` |
...
| no-passive-advisory-bypass-check | static + runtime: no path produces `OperationalProtocolAuthorised` without observed acknowledgement chain | T-204 / T-806b | `informational` from T-806a; `enforced` after T-806b lands | gate predicate active |
...
Recommended next action: run `python tools/agenda_consistency_check.py`, then `/dev-orchestrator` opens T-603 `app.validation_orchestrator` now that T-602 is complete locally.
