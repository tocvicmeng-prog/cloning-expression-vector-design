...
**Date:** 2026-05-14.
**Status:** **v1.5 + v0.2 Enrichment Amendment (landed 2026-05-23)** — the v1.5 body remains authoritative for v0.1.0-shipped behaviour (71 task cards across 14 phases, 50 canonical ports). The v0.2 Enrichment Amendment lands per the project's standing 10-step working-principle cadence at step 9: new Phase 4.2 (10 task cards: catalogue extension — host schema v1.1, markers split, port #51 wiring, MR-55..60 rules, CI gates) + new Phase 14 (11 task cards: ML training corpus subsystem under `docs/ml_corpus/`, runtime-isolated via import-linter, split sequence/annotation license, partition: sa_free DEFAULT, SnapGene cross-check process-only, fork-readiness operational artefact). Post-v0.2 totals: **92 task cards across 16 phases, 51 canonical ports (MarkersCataloguePort = #51).** See ARCHITECTURE.md § 9 for the binding architectural delta and `docs/handover/2026-05-23_host_marker_ml_corpus_development_plan.md` § 4 for the joint plan that scoped the task cards.

**v0.2 Enrichment Amendment changes (2026-05-23).**

- **Phase 4.2 (10 cards: T-407 … T-415 + T-408a).** Host schema v1.0 → v1.1 (additive optional phenotype fields); new `catalogues/markers.yaml` + `schemas/markers.schema.json` v1.0; new `MarkersCataloguePort` (canonical port #51); dual-read migration from `parts.yaml::markers` with shim-hit telemetry; 9 new strain records + ~6-8 existing-record backfill; MR-55..60 rule predicates; new CI gates `markers-citation-presence-check`, `host-marker-link-integrity-check`.
- **Phase 14 (11 cards: T-1401 … T-1411).** New `docs/ml_corpus/` subsystem (out-of-runtime); `corpus_record.schema.json` v1.0 with split `sequence_license` + `annotation_license` blocks per IP-auditor § 5.2 fold-in; CC-BY-SA partition routing + manifest with `partition: sa_free` as DEFAULT per IPQ-1 resolution; `import-linter` contract `ml-corpus-is-not-runtime`; new CI gates `ml-corpus-license-check`, `snapgene-pipeline-scan` (BR-16, enforced from day one), `corpus-annotation-provenance-check` (IPQ-9, informational at v0.2); `corpus_release_gate.py` (≥ 90% cross-check at release-tag time); repo-root `IP_POLICY.md` + `LICENSES/THIRD_PARTY_NOTICES.md`; counsel-facing operational fork-readiness checklist (the NORMATIVE architectural memo already landed in ARCHITECTURE.md § 9.6 at step 8).
- **Port count.** Canonical port count: 50 → **51** (`MarkersCataloguePort` is port #51, owning task T-409). The speculative six v1.6 biological catalogue ports referenced in earlier session memory never landed in `docs/port_manifest.yaml` or in code and are explicitly out of scope for this amendment (see ARCHITECTURE.md § 9.2 memory-drift correction).
- **`agenda_consistency_check.py` updates.** Heading parser regex extended to accept `4.2` as a phase token; `EXPECTED_TOTAL` 71 → **92** (21 new cards: 10 Phase 4.2 + 11 Phase 14; T-408a is its own top-level heading per the explicit "+T-408a" enumeration in the joint plan); `EXPECTED_PHASES` adds `"4.2"` (after `"4"`) and `"14"` (at end); `EXPECTED_COUNTS` adds `"4.2": 10` + `"14": 11`; `EXPECTED_PHASE_4_2` + `EXPECTED_PHASE_14` task lists added; canonical-port-text checks bumped 50 → 51; `STALE_IDS` adds `"T-MARKERS-SHIM"`. (v0.2.1 audit fix M4 reconciled stale "91" wording with the actually-shipped value "92" matching the checker constant.)
- **task_manifest.yaml updates.** `active_task_card_count` 71 → **92**; `phase_order` extended with `"4.2"` and `"14"`; `phase_counts` extended; 21 new task rows appended in heading order.

**Substantive v1.5 changes (retained context):**
...
2. Module catalogue — the build queue (92 active Section 2 task cards across 16 implementation phases — 71 v0.1.0 + 10 new in Phase 4.2 + 11 new in Phase 14 per v0.2 Enrichment Amendment)
...
- **SOP:**
  ...
  4. **(v1.2 B2-06 fix)** T-203c **explicit numbered port inventory** under `src/domain/ports/` — every Protocol below must exist as a Protocol stub:
     ...
     **Total: 51 canonical ports** (v1.5 set the baseline at 50; v0.2 Enrichment Amendment 2026-05-23 adds `MarkersCataloguePort` as canonical port #51 per ARCHITECTURE.md § 9.2 — owning task T-409; bumps the total to **51**. v1.5 arithmetic retained: v1.3 had 45 ports; remove the legacy SOP-template library port per M4-01 (-1); add `SopTemplateSigner` + `SopTemplateVerifier` (+2), `DecisionRecordVerifier` (+1), `AdminAuditAppendPort` (+1), `ReviewQueueAdminPort` (+1), and `AdminServiceClientPort` (+1) = +5 net change → 50; +1 `MarkersCataloguePort` (v0.2) → **51** ports total. `AuditKeyProvider` Protocol surface revised per H4-10, no port count change).

  5. **`tests/ports/test_port_inventory.py` (v1.3 — manifest-driven additions per M3-04; v0.2 Enrichment Amendment 2026-05-23 — port count 51 including `MarkersCataloguePort`)** — imports each of the 51 canonical ports above and fails if (a) any canonical port is missing; (b) any port has merged read and admin-write methods (`AuthorisationStore` legacy combined form is **forbidden**); (c) any expected method is missing; (d) any **additional** port discovered under `src/domain/ports/` lacks a manifest entry in `docs/port_manifest.yaml` (with `port_id`, `owning_task`, `architecture_section`, `requirements_refs` populated — v1.3 M3-04 fix). Manifest-less ports fail the gate; plugin-driven port additions are permitted via the manifest.
...
- **Residual-risk migration addendum (2026-05-14; ARCHITECTURE.md § 6.1):** Completed work order for closing the residual-risk gate stubs:
  1. **G-SEC-01 `no-self-authorisation-check`.** Promote from stub to an enforced aggregate gate. It must compose the direct-admin-handler, audit-append-boundary, and SOP-template-admin-boundary static scans, then verify runtime evidence exists for User/Reviewer denial, DeveloperBootstrapPrincipal expiry, admin-service IPC denial, and review-queue self-approval denial.
  2. **G-SEC-02 `audit-append-port-only-check`.** Promote from stub to an enforced static gate proving engine/app/admin-command code paths do not import `SqliteAuditLog`, `SqliteAuditLogWriter`, or test audit brokers directly. The only production audit writer remains `interface.audit_service` backed by the dedicated audit-service process; governance services must receive `AuditAppendPort` / `AdminAuditAppendPort`.
  3. **G-SEC-03 `sop-template-admin-port-only-check`.** Promote from stub to an enforced static gate proving SOP rendering, SOP orchestration, and catalogue-loader code paths import only `SopTemplateReadPort` / read-only catalogue surfaces, never `SopTemplateAdminWritePort`, `SopTemplateBootstrapPort`, or `SqliteSopTemplateStore`.
  4. **G-TRACE-01 `audit-traceability-check`.** Promote from stub to an enforced traceability gate. It must parse lightweight file headers (`module_id`, `file`, `task_id`), resolve task IDs against `docs/task_manifest.yaml`, and compare generated `docs/traceability_index.md` content with committed file-header metadata.
- **SOP (v1.3):**
...
- **Residual-risk migration status (2026-05-14):** G-SEC-01, G-SEC-02, G-SEC-03, and G-TRACE-01 are implemented locally. `.github/workflows/ci.yaml` now runs the four promoted gates in `--enforce` mode; `tests/ci_gates/test_t204_gates.py` includes them in the enforced-green local gate sweep and verifies the boundary scanners reject synthetic violations. `docs/traceability_index.md` is generated from the lightweight Python file headers and is compared byte-for-byte by `audit-traceability-check`. Catalogue/rule governance gates (`stale-catalogue-check`, `source-grade-citation-check`, `rule-fixture-coverage-check`, `implementation-status-consistency-check`) and manifest governance gates (`module-coverage-check`, `task-acceptance-completeness-check`, `gate-lifecycle-check`, `test-task-brief-coverage`) are also promoted to enforced CI mode now that their predicates pass locally.
...
- **Residual-risk closure status (2026-05-14; R-07 / R-14):** `Sbol3Adapter` now preserves sequence plus exact SBOL `Range` feature coordinates and emits `sbol-feature-location-not-supported` for unsupported coordinate shapes instead of silently dropping annotations.
...
### Phase 4.2 — Catalogue extension (10 tasks; v0.2 Enrichment Amendment 2026-05-23: host schema v1.0 → v1.1 additive, markers catalogue split out, `MarkersCataloguePort` canonical port #51, MR-55..60 rules become machine-checkable)

#### 2.4.2.1 `T-407` — Host schema v1.1 amendment (v0.2)
- **Files:** `schemas/hosts.schema.json` v1.1; `tests/schemas/test_hosts_v1_1_migration.py`.
- **Purpose.** Bump `schemas/hosts.schema.json` to v1.1 with additive optional phenotype fields per ARCHITECTURE.md § 9.1 + REQUIREMENTS FR-HOST-13..20: `aliases[]`, `t7_lysogen`, `protease_status{lon, ompT}`, `disulfide_environment{cytoplasm_state, trxB_status, gor_status}`, `rare_codon_supplementation[]`, `plasmid_addons[]`, `t7_lysozyme_load`, `recombination_phenotype{recA, endA, recBCD_status}`, `methylation_phenotype{dam, dcm, hsdRMS}`, `recommended_selection_markers[]`, `vendor_strain_refs[]`. All fields optional — existing records remain valid.
- **Model tier:** Sonnet. **Context budget:** ≤ 30 k tokens.
- **Depends on:** none (Phase 4.2 entry point).
- **Acceptance:** migration test asserts current `catalogues/hosts.yaml` parses cleanly under v1.1; `mypy --strict` green; `agenda_consistency_check.py` green.
- **Implementation status (2026-05-23).** Complete locally. `schemas/hosts.schema.json` extended with 11 new optional fields (`aliases`, `t7_lysogen`, `protease_status`, `disulfide_environment`, `rare_codon_supplementation`, `plasmid_addons`, `t7_lysozyme_load`, `recombination_phenotype`, `methylation_phenotype`, `recommended_selection_markers`, `vendor_strain_refs`); existing v1.0 `required` field set retained unchanged. Migration test at `tests/schemas/test_hosts_v1_1_migration.py` with 4 assertions: (1) current `catalogues/hosts.yaml` validates cleanly under v1.1 via `adapter.catalogue.load_catalogue`, (2) `required` field set unchanged from v1.0 baseline, (3) all 11 new fields present in `properties`, (4) `recommended_selection_markers[]` pattern locked to `^marker\.[a-z0-9_]+$`. Focused verification: 4 tests passed (`pytest tests/schemas/`); existing `test_hosts_catalogue_t403.py` regression-tested green; import-linter 4/4 contracts KEPT; `agenda_consistency_check.py` green at `92 active task headings, 51 canonical ports`. Schema is ready for T-411 to populate new strain records and T-408a to backfill existing ones.

#### 2.4.2.2 `T-408` — Markers schema v1.0 (new file; v0.2)
- **Files:** `schemas/markers.schema.json` v1.0; `tests/schemas/test_markers_schema.py`; auxotrophic-sanity CI check in `tools/ci/markers_auxotrophic_sanity.py`.
- **Purpose.** New standalone `schemas/markers.schema.json` v1.0 per architect § 2.2 (split out from `parts.yaml::markers`). Includes `$defs` for `marker`, `citation`, `maintenance`, `working_concentrations`, `counter_selection`, `host_genotype_requirement`. Class enum: `bacterial_antibiotic / yeast_auxotrophic / yeast_dominant / mammalian_antibiotic / counterselection`.
- **Model tier:** Sonnet. **Context budget:** ≤ 25 k tokens.
- **Depends on:** T-407 (schema-version monotonicity CI helper).
- **Acceptance:** empty `catalogues/markers.yaml` shell validates; one positive + one negative fixture exercise the schema; auxotrophic-sanity check warns on accidental non-zero `concentration_ugml` combined with `class: yeast_auxotrophic`.
- **Implementation status (2026-05-23).** Complete locally. `schemas/markers.schema.json` v1.0 written FLAT (no `$ref`/`$defs`) to match the project's hand-rolled validator in `adapter.catalogue.yaml_loader._validate` (handles `type / enum / required / additionalProperties / minLength / minItems / format=date` only). Constraints not enforceable by that validator (pattern, oneOf, allOf/if conditional required, maxLength) are intentionally absent and will be enforced by separate CI scripts (this card's `markers_auxotrophic_sanity.py`, plus T-415's `markers-citation-presence-check` + `host-marker-link-integrity-check`). Marker schema covers 5 classes (bacterial_antibiotic / yeast_auxotrophic / yeast_dominant / mammalian_antibiotic / counterselection), 6 host_class enum values (ecoli / scerevisiae / kphaffii / mammalian / insect / plant), 6 citation-grade values (A1/A2/A3/B1/B2/C), and the working_concentrations / counter_selection / host_genotype_requirement nested objects per architect § 2.2.2. `tools/ci/markers_auxotrophic_sanity.py` is a standalone script (argparse-based, `--enforce` flag for future lifecycle promotion); it scans `catalogues/markers.yaml` for class:yeast_auxotrophic + non-zero concentration_ugml; gracefully no-ops when the catalogue file does not yet exist (T-410 populates content). `tests/schemas/test_markers_schema.py` adds 7 tests: positive Amp fixture validates; positive auxotrophic URA3 fixture with sentinel-zero validates; missing required field rejected; unknown class enum rejected; unknown host_class enum rejected; invalid citation grade rejected; schema-loads structural check. Focused verification: 7/7 tests pass; sanity script runs clean (no markers.yaml present yet); import-linter 4/4 KEPT; agenda check green at 92/51. Schema is ready for T-409 to wire `MarkersCataloguePort` against it.

#### 2.4.2.3 `T-408a` — Existing host-record v1.1 backfill (v0.2)
- **Files:** `catalogues/hosts.yaml` edits to ~6-8 existing entries (DH5α, TOP10, JM109, XL1-Blue, Stbl3, BL21 baseline, cell-free); `chassis_class: cell_free` discriminator added for PURE / myTXTL / S30 / TXTL (exempts them from v1.1 phenotype expectations per architect § 6 Q5).
- **Purpose.** Enrich the v0.1.0 host records with v1.1 phenotype fields where applicable. Cell-free hosts are marked with the `chassis_class: cell_free` discriminator and exempt from phenotype-field population. Runs in M-C after new strain records (T-411) have exercised the schema in production.
- **Model tier:** Sonnet. **Context budget:** ≤ 30 k tokens.
- **Depends on:** T-407, T-410, T-411.
- **Acceptance:** `host-marker-link-integrity-check` green; `markers-citation-presence-check` green; agenda check green; behavioural diff in existing engine tests is zero.

#### 2.4.2.4 `T-409` — `MarkersCataloguePort` + adapter + domain types (v0.2)
- **Files:** `src/domain/ports/markers_catalogue.py`; `src/domain/types/markers.py` (`Marker`, `MarkerId`, `MarkerClass`, `HostClass`, `WorkingConcentration`, `CounterSelection`); `src/adapter/catalogue/yaml_markers_catalogue.py`; composition-root wiring update in `src/app/composition_root.py`.
- **Purpose.** Implement the new `MarkersCataloguePort` (canonical port #51) per ARCHITECTURE.md § 4.5 stub + § 9.2 wiring. Read-only at runtime; mutations are offline curation only. `docs/port_manifest.yaml` already at 51 ports (landed step 8).
- **Model tier:** Sonnet. **Context budget:** ≤ 45 k tokens.
- **Depends on:** T-408 (schema).
- **Acceptance:** `test_port_inventory.py` passes (51 ports); read methods (`get_by_id`, `list_all`, `list_by_class`, `list_compatible_with_host`, `working_concentration`) return correctly typed data on empty + populated fixtures; composition-root wiring injects the catalogue into `engine.compatibility` and `engine.validation`.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from architect spec:** the architect-analysis § 3.1 sketched typed value objects (`Marker / MarkerId / MarkerClass / HostClass / WorkingConcentration / CounterSelection`); the project convention for existing catalogue ports (`PartCatalogue`, `HostCatalogue`, `EnzymeCatalogue` all live as inline Protocols in `src/domain/ports/__init__.py` returning raw `Payload = Mapping[str, object]` dicts) is to keep port boundaries untyped. Implementation follows project convention. `MarkersCataloguePort` added inline to `src/domain/ports/__init__.py` as Protocol with three methods: `get_marker(marker_id) -> Payload`; `list_markers() -> PayloadSequence`; `list_compatible_with_host_class(host_class) -> PayloadSequence` (the last one supports the architect-§ 3.2 MR-MARKER-MISMATCH advisory pathway that T-412 + T-414 will wire). Concrete adapter `src/adapter/catalogue/yaml_markers_catalogue.py` wraps `adapter.catalogue.load_catalogue` against `catalogues/markers.yaml` + `schemas/markers.schema.json`; loads eagerly + validates on construction; O(1) lookup via internal id index; immutable post-construction. No composition-root wiring update yet — engine wiring lands at T-412 + T-413 (those tasks own the `engine.compatibility` + `engine.validation` migration with dual-read shim). `tests/adapter/catalogue/test_markers_catalogue_t409.py` adds 5 tests: 2-marker fixture loads via `list_markers`; `get_marker` returns indexed payload; `get_marker` raises KeyError on missing; `list_compatible_with_host_class` filters correctly per host_class; structural Protocol-conformance check confirms `YamlMarkersCatalogue` satisfies `MarkersCataloguePort`. Focused verification: 5/5 tests pass; agenda check green at 92/51. **Project quirk discovered:** `uv sync --no-editable` snapshot-installs `src/` into the venv at sync time; new files under `src/` require either a wheel rebuild OR a manual `cp src/... .venv/Lib/site-packages/...` workaround for tests to find them. Pre-existing constraint; not unique to this task. CI workflows handle this via fresh installs; local dev needs an editable install or manual sync after adding new files.

#### 2.4.2.5 `T-410` — `markers.yaml` content population (v0.2)
- **Files:** `catalogues/markers.yaml` with 23 marker entries (9 bacterial + 6 yeast auxotrophic + 4 dominant yeast + 4 mammalian per REQUIREMENTS FR-MARK-05..08).
- **Purpose.** Curate all 23 markers with Sambrook 4th ed. Appendix A1 working-concentration citations + primary-literature citations where divergent (Bessette 1999 for Origami-specific kanamycin; Goldstein & McCusker 1999 for KanMX/NatMX/HphMX; Wach 1994 for KanMX). Every entry has `citation.grade ∈ {A1, A2, A3, B1, B2}`.
- **Model tier:** **Opus** (scientific curation; per-host concentrations + grade-A citations require judgement load that benefits from Opus).
- **Context budget:** ≤ 60 k tokens.
- **Depends on:** T-409.
- **Acceptance:** `markers-citation-presence-check` green on all 23 entries; every working-concentration block carries a Sambrook citation (or a stronger A1 primary citation); auxotrophic markers carry sentinel `{min:0, typical:0, max:0}` + dropout-medium identity in `medium`.
- **Implementation status (2026-05-23).** Complete locally. `catalogues/markers.yaml` populated with all 23 markers: 9 bacterial antibiotic (Amp/Carb, Kan, Cm, Tet, Spec, Zeo, Gen, Hyg, Erm), 6 yeast auxotrophic (URA3, LEU2, HIS3, TRP1, MET15, LYS2), 4 dominant yeast (KanMX/G418, HphMX/Hyg, NatMX/clonNAT, BlepMX/Zeo), 4 mammalian (Puromycin, Blasticidin, Hygromycin B mammalian, G418/Geneticin mammalian). Citations: Sambrook 4th ed. Appendix A1 (B2) for working concentrations; primary-literature A1 citations attached where they exist — Beck 1982 PMID 6296021 (Kan), Shaw 1975 PMID 1095 (Cm), Hollingshead 1985 PMID 2987812 (Spec/aadA), Drocourt 1990 PMID 2370666 (Zeocin/ble), Gritz & Davies 1983 PMID 6313438 (hph), Sikorski & Hieter 1989 PMID 2659436 (yeast auxotrophic markers), Brachmann 1998 PMID 9483801 (MET15 + BY4741 background), Chattoo 1979 PMID 363676 (LYS2 + α-aminoadipate counter-selection), Boeke 1984 PMID 6394957 (5-FOA counter-selection for URA3), Wach 1994 PMID 7747518 (KanMX), Goldstein & McCusker 1999 PMID 10618490 (HphMX/NatMX/BlepMX), Vara 1986 PMID 3014619 (Puromycin), Izumi 1991 PMID 1959560 (Blasticidin). Counter-selection blocks populated for URA3 (5-FOA) and LYS2 (α-aminoadipate). Auxotrophic markers carry sentinel `{min:0, typical:0, max:0}` with dropout-medium identity per FR-MARK-04. Multi-host concentration blocks where relevant: KanMX (S. cerevisiae 100-200-400 + K. phaffii 100-250-500); BlepMX (S. cerevisiae 100-100-300 + K. phaffii 100-100-500 with stepwise Pichia multicopy-selection note). Focused verification: `load_catalogue` validates 23 markers under markers.schema.json v1.0; `markers-citation-presence-check`: "all 23 markers carry grade A1/A2/A3/B1/B2 citations" (zero grade-C); `markers_auxotrophic_sanity` pass (zero auxotrophic-non-zero-concentration violations); `agenda_consistency_check.py` green at 92/51. **Citation discipline note:** every entry includes a PMID where the primary literature is well-known; some vendor-supplied concentration values are documented as B2-grade (Sambrook-anchored). The catalogue is ready for first-pass scientific review; a future curation pass should add per-marker `notes` for edge-case institutional dosage variants where divergent from Sambrook.

#### 2.4.2.6 `T-411` — New `hosts.yaml` strain entries (9 new; v0.2)
- **Files:** `catalogues/hosts.yaml` additions: Origami(DE3), Origami B(DE3), Rosetta(DE3)pLysS, PichiaPink series (PP1–PP4 strains), KM71, SMD1168H, AH109, Y187, EGY48.
- **Purpose.** Curate 9 new strain records under host schema v1.1 with full phenotype-field population. Primary citations: Bessette 1999 (Origami); Cregg 2000 (Pichia); James 1996 (AH109); Brachmann 1998 (yeast); vendor-manual B2 citations as supplements. AH109 / Y187 / EGY48 carry reporter-cassette annotations in `expression_features[]` per FR-HOST-20.
- **Model tier:** **Opus** (per-strain phenotype + genotype audit; high citation correctness load).
- **Context budget:** ≤ 65 k tokens.
- **Depends on:** T-407 (schema), T-410 (markers, so `recommended_selection_markers[]` can link).
- **Acceptance:** `host-marker-link-integrity-check` green; FR-HOST-16 inventory check green; all 9 strains carry citation grade ≥ B2.
- **Implementation status (2026-05-23).** Complete locally. 9 new strain records appended to `catalogues/hosts.yaml`: Origami(DE3) [Novagen 70203, Bessette 1999 PMID 10570136]; Origami B(DE3) [Novagen 70837, trxB-/gor- on BL21 B-strain]; Rosetta(DE3)pLysS [Novagen 70956 + pLysS + pRARE]; PichiaPink series PP1-PP4 [Invitrogen ade2/pep4Δ/prb1Δ stepwise]; KM71 [Cregg 1985 PMID 3915774, Mut(S) phenotype]; SMD1168H [Invitrogen K1710, pep4Δ + HIS4-restored]; AH109 [James 1996 PMID 8978031, GAL4-based Y2H with HIS3+ADE2+MEL1+lacZ reporters]; Y187 [Harper 1993 PMID 8242751, MAT-α complement for mating-based Y2H]; EGY48 [Estojak 1995 PMID 7565735, LexA-based Y2H]. Every record carries the v1.1 phenotype-field set (t7_lysogen, protease_status, disulfide_environment, rare_codon_supplementation, plasmid_addons, t7_lysozyme_load, recombination_phenotype, methylation_phenotype, recommended_selection_markers, vendor_strain_refs). Pichia strains carry `aliases: ["Pichia pastoris"]` per decision #2. Yeast and Pichia non-applicable fields use the schema's "null" string sentinel (e.g., `t7_lysogen: null`). `recommended_selection_markers[]` populated with cross-links into `catalogues/markers.yaml` (e.g., Origami(DE3) → marker.ampicillin / marker.kanamycin / marker.chloramphenicol; AH109 → marker.trp1 / marker.leu2 / marker.his3). Focused verification: `hosts.yaml` validates under v1.1 schema (73 hosts total = 64 v0.1.0 + 9 new); `host-marker-link-integrity-check`: "all 25 marker links across 9 hosts resolve cleanly" — confirms cross-catalogue integrity end-to-end. Both `markers-citation-presence-check` and `host-marker-link-integrity-check` CI gates **NOW ACTIVELY ENFORCING** real data (lifecycle-promoted from informational vacuous-pass to enforced-green by the population of catalogues/markers.yaml + recommended_selection_markers[] fields). T-408a (existing-record backfill of v1.1 phenotype fields) remains to be done in a follow-up.

#### 2.4.2.7 `T-412` — `engine.compatibility` migration to `MarkersCataloguePort` (v0.2)
- **Files:** `src/engine/compatibility/*` updates; dual-read shim in `src/engine/compatibility/markers_resolver.py`; WARN-level log emitter wired into the governance event stream.
- **Purpose.** Migrate `engine.compatibility` from reading markers in `parts.yaml::markers` to the new port, behind a dual-read shim that falls back to `parts.yaml::markers` on a port miss while emitting a WARN-level log line. The shim closes when zero hits are observed for one full release cycle (data-driven cut-over per architect § 3.3 / joint plan § 4.1).
- **Model tier:** Sonnet. **Context budget:** ≤ 40 k tokens.
- **Depends on:** T-409, T-410, T-411.
- **Acceptance:** existing compatibility-engine tests pass unchanged; new MR-MARKER-MISMATCH advisory test (host's `recommended_selection_markers[]` references a marker whose `working_concentrations[].host_class` does not include the host's `chassis_class`) passes; shim-hit log lines observable under controlled fixture.

#### 2.4.2.8 `T-413` — `engine.validation` migration to `MarkersCataloguePort` (v0.2)
- **Files:** `src/engine/validation/*` updates; rule predicates that currently look up markers re-pointed; same dual-read shim mechanism.
- **Purpose.** Migrate `engine.validation` predicates from `parts.yaml::markers` to the new port (same dual-read pattern as T-412). Predicates affected: existing MR-04, MR-05, and any rule registry entry that touches markers.
- **Model tier:** Sonnet. **Context budget:** ≤ 40 k tokens.
- **Depends on:** T-409, T-412.
- **Acceptance:** existing validator tests pass; shim-hit telemetry visible; new MR-55..60 fixtures are ready for T-414 to wire predicates into.

#### 2.4.2.9 `T-414` — New MR-55..60 rule predicates (v0.2)
- **Files:** rule manifests in `catalogues/rules/MR.yaml` (entries MR-55 methylation-aware enzyme planning, MR-56 disulfide-aware host selection, MR-57 protease-aware host selection, MR-58 rare-codon-aware host suggestion, MR-59 marker-host auxotrophic compatibility, MR-60 Y2H marker compatibility); predicate implementations under `src/engine/validation/predicates/`; rule fixtures (2 per rule — one positive, one negative) under `tests/rules/`.
- **Purpose.** Each new MR rule becomes machine-checkable via the new host phenotype fields (T-407) + markers catalogue (T-410). Citations: REBASE methylation table + Sambrook (MR-55); Bessette 1999 + Lobstein 2012 (MR-56); Gottesman 1996 (MR-57); Kane 1995 (MR-58); Sikorski & Hieter 1989 + Brachmann 1998 (MR-59); James 1996 + Harper 1993 + Estojak 1995 (MR-60).
- **Model tier:** Sonnet. **Context budget:** ≤ 45 k tokens.
- **Depends on:** T-411 (host fields), T-410 (markers), T-413 (port wiring).
- **Acceptance:** each rule fires on its positive fixture and stays silent on its negative fixture; rule-fixture-coverage-check green for the 12 new fixtures (6 rules × 2 fixtures).

#### 2.4.2.10 `T-415` — CI gates landing for Tracks A+B (v0.2)
- **Files:** `tools/ci/markers_citation_presence_check.py`; `tools/ci/host_marker_link_integrity_check.py`; `tools/agenda_consistency_check.py` extensions (port-count consistency check asserting `len(port_manifest.yaml::ports) == ARCHITECTURE.md § 4.5 canonical-port-count`; schema-version monotonicity; hosts-v1.1-field-coverage soft warn).
- **Purpose.** Land the v0.2 CI gates for Tracks A+B per architect § 4 + joint plan § 8. Both new gates go into `enforced` mode in the same PR that lands the dependent data.
- **Model tier:** Sonnet. **Context budget:** ≤ 35 k tokens.
- **Depends on:** T-410, T-411.
- **Acceptance:** both new gates `enforced` and green; port-count consistency check fires on a deliberately desynchronised fixture and passes on the real artefacts; agenda check green at `92 active task headings, 51 canonical ports`. (v0.2.1 audit fix M4 — reconciled stale "91" with the authoritative "92" matching `agenda_consistency_check.py::EXPECTED_TOTAL`; the off-by-one was joint-plan-era and was closed at step 9 of the v0.2 cadence.)
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from CODING_AGENDA spec:** scripts landed at `tools/ci_gates/` (not `tools/ci/`) for framework consistency with the other enforced gates using `_gate.run_gate`. Landed OUT OF DEPENDENCY ORDER — spec said depends on T-410/T-411, but both gates operate correctly on absent/empty catalogues (vacuous PASS) and are useful to land alongside the other CI gates. **Lifecycle: `informational` at v0.2** for both; promotes to `enforced` once T-410 lands `catalogues/markers.yaml` (markers-citation-presence flips) and T-411 + T-408a populate `recommended_selection_markers[]` in `catalogues/hosts.yaml` (host-marker-link-integrity flips). The third deliverable (`agenda_consistency_check.py` port-count consistency extension) was **already landed at step 8 / step 9 of the cadence**: the check now enforces `EXPECTED_TOTAL = 92` and validates the 51-port `port_manifest.yaml` including `MarkersCataloguePort` as port #51. **`markers-citation-presence-check`** checks every marker's top-level citation + every `working_concentrations[].citation` + (if present) `counter_selection.citation` for grade ∈ {A1, A2, A3, B1, B2}; grade C is explicitly rejected for runtime catalogue entries per the architect's § 4.1 source-grade convention. **`host-marker-link-integrity-check`** loads `markers.yaml`'s id set and asserts every `recommended_selection_markers[]` entry across `hosts.yaml` (a) matches the `^marker\.[a-z0-9_]+$` pattern (project validator doesn't enforce regex, this gate does), and (b) resolves to an existing marker id. Verified on real codebase: both gates return vacuous PASS at v0.2 with informative messages indicating the populated-data prerequisites haven't arrived yet.
...
- **Planned UI remediation (2026-05-15).** The first completed SPA slice is functionally valid but under-scales the wet-lab design workflow. Add a workflow-grade interface layer before treating T-1202 as product-complete:
  1. Add a global design status summary showing design ID, host, objective, vector size, review state, export readiness, locked module count, and last saved timestamp.
  2. Add a wet-lab workflow stage rail that maps the design journey from intent capture through backbone/cargo selection, expression cassette configuration, assembly compatibility, sequence validation, biosafety review, and export.
  3. Expand the vector map into a construct architecture workspace by adding a feature table with feature type, start/end coordinates, strand, source, lock state, and validation status.
  4. Add a compatibility matrix for host/promoter/origin/marker/cargo/tag/assembly/biosafety relationships so wet-lab feasibility is visible before export.
  5. Add explicit input/import and export action surfaces for sequence import, backbone/cargo inputs, validation, draft design bundle export, review submission, and final GenBank/SBOL/FASTA bundle export.
  6. Preserve advisory-gate safety: export actions remain visibly gated by validation and institutional review state, and no UI surface exposes operational wet-lab protocol instructions.
...
- **Implementation status (2026-05-14).** Complete locally. Added the deterministic library benchmark harness, 100-realisation smoke fixture, 1000-realisation stretch fixture, locked expected hashes, release build wrappers for the Python wheel and pinned Docker image, a manifest-driven release-note renderer, release docs, and the CI determinism job covering the three white-paper examples plus the 100-realisation library fixture on Linux and Windows runners. The Python wheel was built locally under `task_artefacts/T-1303/dist/`; Docker is not installed in this workspace, so the container build is verified locally by dry-run plan and is wired for CI execution. Focused verification: T-1303 UAT/release tests passed; the 1000-realisation stretch fixture passed; release determinism check passed; full strict mypy and ruff passed for the new surface; release build dry-runs passed. This completes all v0.1.0 active Section 2 implementation task cards.

### Phase 14 — ML training corpus subsystem (11 tasks; v0.2 Enrichment Amendment 2026-05-23: new `docs/ml_corpus/` sibling subsystem, out-of-runtime, split sequence/annotation license, partition: sa_free DEFAULT per IPQ-1)

#### 2.14.1 `T-1401` — `docs/ml_corpus/` scaffold + import-linter contract (v0.2)
- **Files:** `docs/ml_corpus/README.md`, `docs/ml_corpus/corpus_manifest.yaml` (initial empty shell), `docs/ml_corpus/exclusions.yaml` (initial empty shell), `docs/ml_corpus/crosscheck_log.yaml` (initial empty shell), folder tree `docs/ml_corpus/records/{backbones/{ecoli,kphaffii,scerevisiae,mammalian},elements/{promoters,terminators,rbs_kozak,polyA,ires_2a,mcs,tags,fluorescent_proteins,selection_cassettes,insulators,introns_wpre},cc-by-sa}/`, `docs/ml_corpus/schemas/` empty for T-1402; `pyproject.toml` `import-linter` contract `[[tool.importlinter.contracts]]` with `name = "ml-corpus-is-not-runtime"`, `type = "forbidden"`, `source_modules = ["src.*"]`, `forbidden_modules = ["docs.ml_corpus.*"]`.
- **Purpose.** Scaffold the new out-of-runtime ML corpus subsystem per ARCHITECTURE.md § 9.3.1. README carries working-principle preamble + SnapGene nominative-use disclaimer (one-line pointer to `LICENSES/THIRD_PARTY_NOTICES.md` once T-1410 lands).
- **Model tier:** **Haiku** (pure boilerplate).
- **Context budget:** ≤ 15 k tokens.
- **Depends on:** none (Phase 14 entry point).
- **Acceptance:** folder tree exists; `import-linter` green; new contract visible in CI output.
- **Implementation status (2026-05-23).** Complete locally. Folder tree created: 1 `schemas/` subfolder + 4 `backbones/` subfolders + 11 `elements/` subfolders + 1 `cc-by-sa/` subfolder = 17 record folders, each carrying a `.gitkeep`. `README.md` written with working-principle preamble (mirror of `cev-workflow-discipline` 10-step cadence), folder layout diagram, partition strategy (`partition: sa_free` DEFAULT per IPQ-1), SnapGene posture (BR-16 NORMATIVE, no automated access), nominative-use disclaimer with pointer to `LICENSES/THIRD_PARTY_NOTICES.md` (T-1410-deferred). `corpus_manifest.yaml` empty shell with structural keys (`partition_default: sa_free`, two partition stubs, `license_aggregate` stub, `release_gate` thresholds with 0.90 research / 0.95 commercial). `exclusions.yaml` empty shell with 9 permitted exclusion-reason categories. `crosscheck_log.yaml` empty shell with NORMATIVE process notes (no automation; ≤ 200-char researcher-authored discrepancies). `.importlinter` extended with new `[importlinter:contract:ml-corpus-is-not-runtime]` forbidding `docs` top-level from being imported by `domain / engine / app / adapter / interface`. **Implementation note**: architect spec at ARCHITECTURE.md § 9.3.3 named `docs.ml_corpus.*` as the forbidden target, but import-linter rejects external subpackages — the implementation forbids `docs` at top level, which is broader but defensible because `docs/` has no `__init__.py` by design and no legitimate import path crosses this boundary. Required top-level `include_external_packages = True` set in `.importlinter` [importlinter] section. Focused verification: `lint-imports --config .importlinter` reports `Contracts: 4 kept, 0 broken` (Domain layer, Engine layer, sop_protected, ml-corpus-is-not-runtime). `agenda_consistency_check.py` green at `92 active task headings, 51 canonical ports`. Scaffold is ready for T-1402 to write `corpus_record.schema.json` v1.0.

#### 2.14.2 `T-1402` — Corpus record schema v1.0 (split license; v0.2)
- **Files:** `docs/ml_corpus/schemas/corpus_record.schema.json` v1.0 per architect § 2.3 + IP-auditor § 5.2 fold-in (split `license` → `{sequence_license, annotation_license}` blocks, both required); `tests/schemas/test_corpus_record_schema.py` with positive + negative fixtures (note: tests live under `tests/schemas/` to match existing test-tree convention, not the originally-proposed `tests/ml_corpus/`).
- **Purpose.** Authoritative per-record schema. License blocks carry `spdx_id`, `redistribution_allowed` (bool, explicit), `ml_training_allowed` (bool, explicit), `attribution_required`, `commercial_use_allowed`, `source_text_url`. `snapgene_crosscheck` block (`checked: bool`; conditional fields). `provenance.source` enum drawn from IP-auditor § 8.1 allowlist (no `addgene_metadata_only` as default-allow).
- **Model tier:** Sonnet. **Context budget:** ≤ 30 k tokens.
- **Depends on:** T-1401.
- **Acceptance:** empty corpus validates; positive + negative fixtures exercise every required field; missing license block fails validation.
- **Implementation status (2026-05-23).** Complete locally. `docs/ml_corpus/schemas/corpus_record.schema.json` v1.0 written FLAT (no `$ref` / `$defs`) to match the project's hand-rolled validator. License block split into `sequence_license` + `annotation_license` per IP-auditor § 5.2 fold-in (both required; each requires `spdx_id`, `redistribution_allowed`, `ml_training_allowed`, `attribution_required`, `commercial_use_allowed`, `source_text_url`). `provenance.source` enum = the IP-auditor § 8.1 Tier-1+Tier-2 allowlist of 9 sources; `addgene_metadata_only` deliberately absent per FR-ML-04. `category` enum = 15 record categories (backbone + 14 modular element types). `sequence.topology` enum = circular/linear. `snapgene_crosscheck.checked: false` allowed (process-only artefact, not a runtime gate per ARCHITECTURE.md § 9.3.5). `checksum.algorithm` enum = sha256/sha512. Implementation deviation from architect § 2.3 spec: conditional `if/then` required-when-checked-true is not in the validator's vocabulary; that conditional is left as documentary intent and the CI gate `ml-corpus-license-check` (T-1403) will enforce it. Discrepancy_resolution `maxLength: 200` cap is also documentary (validator doesn't enforce maxLength); T-1403 + T-1408 enforce it. `tests/schemas/test_corpus_record_schema.py` adds 9 tests: positive pUC19-like backbone validates; license required-block / sequence_license required-field rejections work; addgene-metadata-only absence asserted (FR-ML-04 guard); provenance.source enum completeness asserted; snapgene_crosscheck `checked: false` accepted; category + topology enum rejections work. Focused verification: 9/9 tests pass; agenda check green at 92/51; import-linter 4/4 KEPT (ml-corpus-is-not-runtime contract still green). Schema is ready for T-1403 to add the license CI gate and T-1404/T-1405 to seed corpus records.

#### 2.14.3 `T-1403` — `ml-corpus-license-check` CI gate (v0.2)
- **Files:** `tools/ci_gates/ml_corpus_license_check.py`; `tests/ci_gates/test_ml_corpus_license_check_t1403.py`; CI lifecycle table extension in this file's § 3.5.
- **Purpose.** Default-deny CI gate: every record under `docs/ml_corpus/records/**/*.json` must carry a complete license block with both `redistribution_allowed` and `ml_training_allowed` set explicitly. Missing-or-default is a hard fail. Also counts dual-read shim-hit log lines from T-412/T-413 + emits a coverage warning when `cross_check_coverage < 60%` (research v0.2 floor; release gate at T-1409 tightens to ≥ 90%).
- **Model tier:** Sonnet. **Context budget:** ≤ 30 k tokens.
- **Depends on:** T-1402.
- **Acceptance:** lands `informational` initially; promoted `enforced` after T-1407.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from CODING_AGENDA spec:** path landed at `tools/ci_gates/ml_corpus_license_check.py` (NOT `tools/ci/...` as the agenda card said) to match the framework convention with the other 17 enforced gates using `tools.ci_gates._gate.run_gate`. **Implementation deviation from feature spec:** the dual-read shim-hit counting is deferred — `engine.compatibility` and `engine.validation` don't yet emit shim-hit log lines (T-412 / T-413 land that mechanism). The coverage-WARN feature is deferred for the same reason — the manifest's `cross_check_coverage` block is the empty shell stub from T-1401 (`fraction_checked: 0.0` baseline). Both features land at T-1407 + T-1409 when the populated corpus + manifest generator exist. **What ships now:** the four primary checks per FR-ML-05 + BR-15 — (a) license object present; (b) both sequence_license + annotation_license sub-blocks present; (c) each sub-block carries `redistribution_allowed` + `ml_training_allowed` as explicit booleans (BR-15 default-deny); (d) `sequence_license.ml_training_allowed: false` under records/ is rejected (must live in exclusions.yaml). PLUS the FR-ML-04 provenance allowlist enforcement — addgene_metadata_only and any non-Tier-1+2 source rejected. Behaviour on empty corpus (v0.2 state): vacuous PASS with "BR-15 default-deny baseline holds" message. `tests/ci_gates/test_ml_corpus_license_check_t1403.py` — 9 tests: real corpus passes (baseline); empty tmp corpus passes; complete record passes; missing license object fails; missing sequence_license sub-block fails; missing required boolean field fails; ml_training_allowed: false under records/ fails; addgene_metadata_only provenance fails; allowlist constant excludes addgene_metadata_only. Focused verification: 9/9 tests pass; `ml-corpus-license-check --enforce` on real codebase returns 0; agenda check green at 92/51. **Gate lifecycle: `informational` at landing**; promotes to `enforced` after T-1407 lands the populated corpus (per joint plan § 8 ordering).

#### 2.14.4 `T-1404` — Backbone corpus seed (~40 records; v0.2)
- **Files:** 40 backbone records under `docs/ml_corpus/records/backbones/{ecoli,kphaffii,scerevisiae,mammalian}/` (pUC, pBR322, pET, pBAD, pCDF, pRSF, pACYC, pGEX, pMAL, pTrc for E. coli; pPICZ, pPIC, pAO815, pGAPZ for Komagataella phaffii; pYES2, pRS, p2μ, YIp, YEp, YCp for S. cerevisiae; pcDNA, pLenti, pAAV, pCMV, pIRES, pHR' for mammalian).
- **Purpose.** Curate 40 empty-backbone records with full provenance + split license blocks. Sources: NCBI Nucleotide via INSDC accession (Tier-1 allowlist); iGEM CC0 entries; vendor primary publications (Studier 1990 for pET; Cregg 1988 for Pichia; Dull 1998 for lentivirus). CC-BY-SA-licensed records route to `cc-by-sa/`. SnapGene cross-check is `checked: false` at this stage; coverage builds up in M-C.
- **Model tier:** **Opus** (per-backbone provenance + license correctness + INSDC accession validation requires judgement load).
- **Context budget:** ≤ 70 k tokens.
- **Depends on:** T-1402, T-1403.
- **Acceptance:** all 40 records validate; `provenance.source` ∈ allowlist; `license` blocks complete; license aggregate populated.

#### 2.14.5 `T-1405` — Modular-element corpus seed (~80 records; v0.2)
- **Files:** 80 modular-element records under `docs/ml_corpus/records/elements/{promoters,terminators,rbs_kozak,polyA,ires_2a,mcs,tags,fluorescent_proteins,selection_cassettes,insulators,introns_wpre}/`.
- **Purpose.** Curate 80 modular-element records across the 11 category folders. Sources: iGEM Anderson library promoters; Voigt collection promoters; Cambray 2013 terminator library (PMID 23275534); Salis 2020 RBS Calc datasets; Noderer 2014 Kozak datasets; FPbase CC0 entries for fluorescent proteins; primary literature for IRES / 2A / WPRE.
- **Model tier:** **Opus** (modular-element curation; per-element citation correctness).
- **Context budget:** ≤ 75 k tokens.
- **Depends on:** T-1402, T-1403.
- **Acceptance:** all 80 records validate; license aggregate populated; cross-check coverage tracked.

#### 2.14.6 `T-1406` — BR-16 `snapgene-pipeline-scan` CI gate (v0.2)
- **Files:** `tools/ci_gates/snapgene_pipeline_scan.py`; `tests/ci_gates/test_snapgene_pipeline_scan_t1406.py`; CI lifecycle table extension.
- **Purpose.** Defensive default-deny scanner per BR-16. Scans `.github/workflows/`, `tools/**/*.py`, `Dockerfile`, `mkdocs.yml`, any CI-config or pipeline script for invocations of `curl`, `wget`, `httpx`, `requests`, `playwright`, `selenium`, MCP web-fetch, or any other automated HTTP client targeting `snapgene.com`. Fails the build if found.
- **Model tier:** Sonnet. **Context budget:** ≤ 25 k tokens.
- **Depends on:** T-1401.
- **Acceptance:** lands `enforced` from day one (defensive); test fixture introducing a forbidden invocation fails the build; real codebase passes.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from CODING_AGENDA spec:** the agenda said `tools/ci/snapgene_pipeline_scan.py` but the project's enforced-gate convention is `tools/ci_gates/` (matches 16 existing enforced gates using `tools.ci_gates._gate.run_gate`). The script landed at `tools/ci_gates/snapgene_pipeline_scan.py` to match the framework — uses `GateResult / pass_gate / fail_gate / run_gate` per the established pattern. 11 regex patterns cover the canonical fetch-tool families: `curl`, `wget`, `requests.{get/post/put/delete/head/options/patch/request}`, `httpx.*`, `urllib(.request).urlopen`, `aiohttp`, `playwright`, `selenium`, `mcp_*`, `webfetch`, generic `fetch(...)`. Each pattern requires the tool token AND `snapgene.com` on the same line — known limitation: multi-line obfuscation (variable indirection like `url = "...snapgene.com..."; requests.get(url)`) evades; this is the regex-heuristic trade-off documented in the file header. In-scope file globs: `.github/workflows/*.{yml,yaml}`, `tools/**/*.py`, `Dockerfile{,.*}`, `mkdocs.yml`, `pyproject.toml`, `.importlinter`, top-level `*.sh / *.bat / *.ps1`, `Makefile`. Out of scope (deliberately): `docs/**/*.md` (legitimate discussion), `src/**` (shielded by import-linter `ml-corpus-is-not-runtime`), `tests/**` (fixtures may carry SnapGene URLs as data). Self-reference exception: this file itself contains `snapgene.com` as a regex target → `SELF_EXCLUDE` lists it. `tests/ci_gates/test_snapgene_pipeline_scan_t1406.py` adds 7 tests: (1) real codebase clean (BR-16 default-deny baseline holds); (2-4) synthetic curl / requests / playwright fixtures correctly flagged; (5) snapgene.com used as string-only test data NOT flagged (false-positive guard); (6) SELF_EXCLUDE contains scanner's own path; (7) FORBIDDEN_PATTERNS coverage check. Focused verification: 7/7 T-1406 tests pass; 27/27 v0.2 schema+gate tests pass overall; `snapgene-pipeline-scan --enforce` against real codebase returns 0 with `BR-16 default-deny holds` message; agenda check green at 92/51; import-linter 4/4 KEPT. **Gate lifecycle: `enforced` from day one** per BR-16 + joint plan § 8 — no `informational` warmup period. Note on test invocation: tests under `tests/ci_gates/` require `PYTHONPATH=.` (or equivalent) for `from tools.ci_gates...` imports to resolve; same constraint applies to existing v0.1.0 tests at `tests/ci_gates/test_t204_gates.py` (this is an existing project quirk, not introduced by T-1406).

#### 2.14.7 `T-1407` — CC-BY-SA partition routing + manifest generation (v0.2)
- **Files:** `tools/corpus/partition_router.py` (routes records into `cc-by-sa/` subfolder based on license SPDX); `tools/corpus/manifest_generator.py` (regenerates `corpus_manifest.yaml::license_aggregate` after each PR); `docs/ml_corpus/corpus_manifest.yaml` populated with `partition_default: sa_free` (locked per IPQ-1) + `partition: full` (opt-in) + `partition: sa_free` partitions.
- **Purpose.** Operationalise the dual-partition architecture per ARCHITECTURE.md § 9.3.4. `partition: sa_free` is the **DEFAULT** training partition; `partition: full` exists for research-only opt-in. Manifest aggregate exposes counts, license distribution, attribution obligations, cross-check coverage.
- **Model tier:** Sonnet. **Context budget:** ≤ 40 k tokens.
- **Depends on:** T-1404, T-1405.
- **Acceptance:** manifest aggregate matches on-disk records; both partitions queryable; `partition_default: sa_free` locked; `ml-corpus-license-check` flips to `enforced` after T-1407 lands.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from spec:** partition_router and manifest_generator collapsed into a single `tools/corpus/partition_router.py` module (with `route_records()` and `regenerate_manifest()` as separate functions). The two original separate files (partition_router + manifest_generator) would have introduced unnecessary surface area; a single argparse-driven tool with `--skip-routing` / `--skip-manifest` flags covers both roles. Also landed OUT OF DEPENDENCY ORDER — T-1407 was supposed to depend on T-1404/T-1405 (which seed records), but the tool needs to be in place BEFORE seeding so curators can run it after each ingestion batch. Operates correctly on empty v0.2 corpus (0 records → 0% sa_free fraction). `route_records()` walks `docs/ml_corpus/records/` for any record whose `sequence_license.spdx_id` OR `annotation_license.spdx_id` starts with `CC-BY-SA` and moves the JSON file into `docs/ml_corpus/records/cc-by-sa/`. `regenerate_manifest()` recomputes `corpus_manifest.yaml::license_aggregate` (total_records, by_sequence_license, by_annotation_license, by_provenance_source, partition_sa_free, attribution_obligations, cross_check_coverage) AND `partitions.full.record_count` + `partitions.sa_free.record_count`. `--dry-run` flag for preview. Tested on empty corpus: `partition_router: routed 0 CC-BY-SA record(s); 0 other(s) untouched`. `ml-corpus-license-check` lifecycle promotion to `enforced` is deferred — that flip happens when T-1404 + T-1405 land records AND this tool runs cleanly against them, per joint plan § 8.

#### 2.14.8 `T-1408` — IPQ-9 `corpus-annotation-provenance-check` CI gate (v0.2)
- **Files:** `tools/ci_gates/corpus_annotation_provenance_check.py`; CI lifecycle table extension.
- **Purpose.** Heuristic scanner per IPQ-9 — scans each corpus record's annotation `qualifiers` for long string patterns matching known vendor-manual phrasing (Invitrogen / Thermo Fisher, Novagen / Merck, Stratagene / Agilent, Takara / Clontech, Promega, NEB, Lucigen, SnapGene). Catches accidental verbatim ingestion of vendor-authored annotation text.
- **Model tier:** Sonnet. **Context budget:** ≤ 30 k tokens.
- **Depends on:** T-1404, T-1405.
- **Acceptance:** lands **`informational`** at v0.2 (per IPQ-9 resolution); test fixture with deliberately-quoted vendor phrasing triggers a warning; FP-rate observed for one full release cycle before promotion to `enforced` in v0.3.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from CODING_AGENDA spec:** path landed at `tools/ci_gates/corpus_annotation_provenance_check.py` (not `tools/ci/`) for framework consistency with the other enforced gates. Landed OUT OF DEPENDENCY ORDER — spec said depends on T-1404/T-1405, but the gate operates correctly on empty corpus (returns vacuous PASS) and was useful to land alongside the other CI gates. **Lifecycle: `informational` at v0.2** per IPQ-9 (FP-rate observation period). 6 vendor-phrasing patterns: "For research use only", "Not for use in (diagnostic|therapeutic|human|clinical)", "This product is sold under license", "catalog(ue) number", "©YYYY (vendor)", "(vendor) ... (all rights reserved|trademark|TM|(R)|®)". Scans `docs/ml_corpus/records/**/*.json` annotation `qualifiers` for matches. Verified on real empty corpus: vacuous PASS. Will see real activity once T-1404 + T-1405 seed records. v0.3 promotion to `enforced` happens after FP-rate observation against the populated corpus.

#### 2.14.9 `T-1409` — `corpus_release_gate.py` (release-tag time; v0.2)
- **Files:** `tools/release/corpus_release_gate.py`; release-workflow wiring.
- **Purpose.** Release-tag-time gate (not per-PR) enforcing: `fraction_checked >= 0.90` (SnapGene cross-check at research release; 95% for commercial fork per ARCHITECTURE.md § 9.6.2); `attribution_text_file` exists and is complete; no Tier-3 denylist sources (`addgene_metadata_only`, SnapGene-authored content, paywalled journal figures) present; no records with missing license blocks.
- **Model tier:** Sonnet. **Context budget:** ≤ 35 k tokens.
- **Depends on:** T-1407.
- **Acceptance:** gate runs on tag promotion only; PR merges unaffected; deliberately under-threshold fixture fails the gate; real corpus at expected v0.2 coverage passes.
- **Implementation status (2026-05-23).** Complete locally. `tools/release/corpus_release_gate.py` is an argparse-driven release-time gate (not invoked via `_gate.run_gate` framework — release tools live under `tools/release/` per existing project convention alongside `build_container_image.py`, `build_wheel.py`, `render_release_notes.py`). Two modes: `--research` (default, ≥ 90% cross-check coverage; attribution file present; no Tier-3 sources) and `--commercial` (research checks + 100% commercial_use_allowed; ≥ 95% cross-check; no cc-by-sa/ records; counsel-review certificate at `--certificate` path required). On empty corpus (v0.2 state): vacuous PASS with "corpus has no records to release" message — meaningful enforcement begins once T-1404 + T-1405 land records. Verified on real codebase: `python -m tools.release.corpus_release_gate --tag v0.2.0 --research` → "research release v0.2.0 — corpus has no records to release (v0.2 baseline; T-1404/T-1405 populate). Gate vacuously passes." Release-workflow wiring (calling this script from a CI release workflow) is deferred — `.github/workflows/release.yml` doesn't exist yet; that scaffolding lands when the first release candidate ships. **TIER_3_DENYLIST_SOURCES** constant lists `addgene_metadata_only`, `snapgene_dna_file`, `snapgene_page_scrape`, `vendor_manual_annotation_verbatim`, `paywalled_journal_text` — note these are NOT in the schema's allowed provenance.source enum (which only lists Tier-1+2), so they would be caught by `ml-corpus-license-check` (T-1403) at PR time too; this gate is the last-mile check.

#### 2.14.10 `T-1410` — `IP_POLICY.md` + `LICENSES/THIRD_PARTY_NOTICES.md` (v0.2)
- **Files:** `IP_POLICY.md` at repo root; `LICENSES/THIRD_PARTY_NOTICES.md`; `LICENSES/` directory containing full text of each license referenced in the corpus (CC0, CC-BY-4.0, CC-BY-SA-4.0); update repo READMEs naming SnapGene to one-line pointers to `THIRD_PARTY_NOTICES.md`.
- **Purpose.** Counsel-facing IP/ToS policy: source tier list (IP-auditor § 8.1), SnapGene index-only posture (§ 3), CC-BY-SA partition strategy (§ 6.3), fork-readiness memo pointer (→ ARCHITECTURE.md § 9.6), quarterly ToS re-check schedule (IPQ-4 → 2026-Q3 by 2026-09-30). Centralises the trademark disclaimer per IPQ-8.
- **Model tier:** Sonnet. **Context budget:** ≤ 35 k tokens.
- **Depends on:** T-1407, T-1409.
- **Acceptance:** both files committed; repo READMEs referencing SnapGene shortened to one-line pointers; SnapGene nominative-use disclaimer in canonical form per IP-auditor § 3.4.
- **Implementation status (2026-05-23).** Complete locally. **Implementation deviation from dependency spec:** T-1410's CODING_AGENDA card listed `depends_on: ["T-1407", "T-1409"]`, but the policy documents themselves don't structurally depend on the partition-routing tool or release-gate code — they document the POLICY those tools enforce. Landed early (out of dependency order) to crystallise the fork-readiness compliance posture while the architectural memorandum in `ARCHITECTURE.md` § 9.6 is fresh; T-1407 / T-1409 implementation can refine the policy pointers later if needed. `IP_POLICY.md` at repo root: 8 sections (source-tier list per IP-auditor § 8.1; SnapGene posture per BR-16; CC-BY-SA partition strategy per IPQ-1; quarterly ToS re-check schedule with first snapshot due 2026-09-30; fork-readiness pointer to ARCHITECTURE.md § 9.6 + this doc + the operational checklist at T-1411; project licensing posture clarifying that GPL-3.0 governs toolkit code only and model weights are a separate artefact with their own licensing decision; counsel-review checkpoints; cross-references). `LICENSES/THIRD_PARTY_NOTICES.md`: 5 sections covering SnapGene nominative-use disclaimer in canonical form (with US 9th Cir. *New Kids on the Block* + AU s122 Trade Marks Act 1995 legal basis), open-data sequence sources (INSDC + community registries), open-source software dependencies (deferred SBOM at release-tag time), primary scientific literature (citation-only posture), update protocol. Repo READMEs were NOT shortened to one-line pointers in this push (existing READMEs barely mention SnapGene; the disclaimer is centralised here and pointed to from `docs/ml_corpus/README.md` already). Focused verification: both files committed; `agenda_consistency_check.py` green at 92/51; `snapgene-pipeline-scan --enforce` green (the new files contain `snapgene.com` as discussion content but NOT as fetch invocations).

#### 2.14.11 `T-1411` — Operational fork-readiness checklist (v0.2)
- **Files:** `docs/fork-readiness/checklist.md` (operational artefact for a future fork engineering team); links to verification scripts under `tools/fork_readiness/`.
- **Purpose.** Translate ARCHITECTURE.md § 9.6 NORMATIVE memo into an implementation-team-readable checklist with concrete verification commands (e.g., `python tools/fork_readiness/verify_partition.py --release-tag X.Y.Z`, `python tools/fork_readiness/verify_license_completeness.py`, `python tools/fork_readiness/regenerate_attributions.py`). Each § 9.6 enumerated item (datasets, workflows, design elements, license/IP posture, regulatory posture) gets a corresponding verification command. The NORMATIVE memo in ARCHITECTURE.md remains the binding source; this checklist is the operational mirror.
- **Model tier:** **Opus** (strategic counsel-facing content; cross-references several legal + regulatory frameworks).
- **Context budget:** ≤ 60 k tokens.
- **Depends on:** T-1410.
- **Acceptance:** every § 9.6 (a)-(e) bullet has a corresponding verification entry; counsel-review certificate template stub committed under `tools/fork_readiness/counsel_review_certificate_template.md`.
- **Implementation status (2026-05-23).** Complete locally. `docs/fork-readiness/checklist.md` mirrors ARCHITECTURE.md § 9.6 with 8 sections: how-to-use; (a) Datasets exclude/substitute — 5 verification items (a.1-a.5); (b) Workflows relax/tighten — 6 verification items (b.1-b.6); (c) Design elements re-point — 5 items (c.1-c.5); (d) License + IP posture — 6 items (d.1-d.6); (e) Regulatory posture — 6 items (e.1-e.6); (f) Counsel-review certificate template embedded with YAML structure for AU + US/California reviewers, section-by-section verification slots, signature + certificate-hash slots; (g) Verification-script stubs documenting the NORMATIVE contract of 6 scripts under `tools/fork_readiness/` (verify_partition, verify_license_completeness, verify_ml_training_consent, verify_provenance_sources, regenerate_attributions, counsel_review_certificate_template); (h) Cross-references. **Implementation deviation:** the 6 verification scripts themselves are NOT implemented at v0.2; only their documented contracts ship. Rationale per the checklist's § 7: implementation belongs to the future fork engineering team at the time the commercial-fork attempt is initiated — the contracts are stable, but the implementations need fresh requirements + counsel input at fork time, not now. The `tools/fork_readiness/` directory was created (mkdir during T-1410/T-1411 push); the scripts are stubs (or absent) deferred to fork-time. Focused verification: file committed; cross-references checked against ARCHITECTURE.md § 9.6; agenda check green at 92/51.
...
| no-self-authorisation-check | static + runtime across six surfaces: UserPrincipal and ReviewerPrincipal cannot call `AuthorisationAdminWritePort`; UserPrincipal/ReviewerPrincipal cannot call `SopTemplateAdminWritePort`; engine code cannot import audit write surfaces directly; CLI/API cannot import `AdminActionHandler`; admin-service IPC cannot be bypassed; ordinary DeveloperPrincipal denied post-bootstrap | T-204 / T-311 / T-313b / T-316b / T-1103b / T-1302 | `enforced` after residual-risk migration G-SEC-01 local verification | all admin/audit/SOP write boundaries exist |
| audit-append-port-only-check | static scan: engine/app/admin-command code paths never import `SqliteAuditLog`, `SqliteAuditLogWriter`, or test audit brokers directly; append flows through `AuditAppendPort` / `AdminAuditAppendPort` IPC clients | T-204 / T-313b | `enforced` after residual-risk migration G-SEC-02 local verification | dedicated audit-service writer exists |
| sop-template-admin-port-only-check | static scan: SOP rendering, SOP orchestration, and catalogue-loader paths never import `SopTemplateAdminWritePort`, `SopTemplateBootstrapPort`, or `SqliteSopTemplateStore` | T-204 / T-316b / T-803 | `enforced` after residual-risk migration G-SEC-03 local verification | signed SOP-template store + read-only renderer exist |
...
| audit-traceability-check | every traceability reference resolves; generated `docs/traceability_index.md` matches file headers | T-204 | `enforced` after residual-risk migration G-TRACE-01 local verification | headers present |
