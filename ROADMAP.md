...
**Status:** Phase 0 + Phase 1 + Phase 2 + Phase 3 + Phase 4 + Phase 5 + Phase 6 + Phase 7 complete locally. Phase 8a is in progress: T-801 delivered the risk-advisory report generator, T-802 delivered the always-renderable design-plan generator and deterministic renderers, T-804 delivered first-class control-set generation, and T-805a delivered pre-screening `DraftDesignBundle` orchestration with design-stream render events and no gated operational artefacts. T-806a `app.advisory_acknowledgement` is the next hand-off.
