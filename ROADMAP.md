...
**Status:** Phase 0 + Phase 1 + Phase 2 + Phase 3 + Phase 4 + Phase 5 complete locally, and Phase 6 is complete locally through T-606. T-601a..k delivered deterministic local biology adapters, fixture fakes, adapter manifests, and calibration policies; T-602 delivered pure biology-dependent predicates over those metric payloads; T-603 delivered validation metric pre-computation and derivation-environment-bound cache orchestration; T-606 delivered event-sourced design-service use cases and phase-local pending states. T-607 `app.decision_tree` is the next hand-off.
