...
**Status:** Phase 0 + Phase 1 + Phase 2 + Phase 3 + Phase 4 + Phase 5 + Phase 6 + Phase 7 complete locally. Phase 8a is in progress: T-801 delivered the `engine.risk_classification` risk-advisory report generator over `catalogues/risk_advisories.yaml`, with deterministic report hashing, graded-citation mapping, severity ordering, and inferred trigger tags from biosafety, screening, provenance, export, vector, and cargo metadata. T-802 `engine.design_plan` is the next hand-off.
